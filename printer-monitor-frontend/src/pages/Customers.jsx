...
const Customers = () => {
  ...
  const openAddModal = () => {
    ...
    setNewName(''); setNewContact(''); setError(null);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(`http://localhost:5000/api/customers/${editingId}`, { name: newName, contact_info: newContact });
      } else {
        await axios.post('http://localhost:5000/api/customers', { name: newName, contact_info: newContact });
      }
