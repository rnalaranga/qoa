...
const CustomerAssignmentModal = ({ printer, customers, onClose, onAssign }) => {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (isCreatingNew) {
        ...
        const res = await axios.post('http://localhost:5000/api/customers', { name: newCustomerName });
        customerId = res.data.id;
      }
      if (!customerId) { setError('Please select or create a customer'); setLoading(false); return; }
      await axios.put(`http://localhost:5000/api/printers/${printer.ip_address}/assign`, { customer_id: customerId });
      onAssign();
      onClose();
    } catch (err) {
  ...
  const handleUnassign = async () => {
    setLoading(true);
    try {
      await axios.put(`http://localhost:5000/api/printers/${printer.ip_address}/assign`, { customer_id: null });
      onAssign();
      onClose();
    } catch {
